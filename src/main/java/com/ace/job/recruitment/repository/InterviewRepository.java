...
public interface InterviewRepository extends JpaRepository<Interview, Long>, DataTablesRepository<Interview, Long> {
	...
	// Sửa lỗi CURRENT_DATE thành CURRENT_TIMESTAMP
	@Query("SELECT i FROM Interview i WHERE (i.status = true AND i.endDate IS NOT NULL AND i.endDate < CURRENT_TIMESTAMP) OR (i.status = true AND i.endDate IS NULL AND i.startDate < CURRENT_TIMESTAMP)")
